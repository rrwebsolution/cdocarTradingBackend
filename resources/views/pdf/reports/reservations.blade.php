...
<head>
    ...
    <style>
        ...
        h2 { color: #1d4ed8; font-size: 13px; margin-top: 4px; }
...
<body>
    @include('pdf.partials.header', ['title' => 'Reservation Report', 'logoBase64' => $logoBase64 ?? null])
        ...
        <tbody>
            @forelse ($rows as $row)
                <tr>
                    ...
                    <td>{{ $row->reserved_at?->format('M d, Y') ?? 'N/A' }}</td>
                    <td>{{ $row->expires_at?->format('M d, Y') ?? 'N/A' }}</td>
                </tr>
            @empty
                <tr>
                    <td class="muted" colspan="8">No records match the selected filters.</td>
                </tr>
            @endforelse
        </tbody>
    </table>
