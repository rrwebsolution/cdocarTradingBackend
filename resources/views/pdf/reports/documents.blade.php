...
<head>
    ...
    <style>
        ...
        h2 { color: #6d28d9; font-size: 13px; margin-top: 4px; }
...
<body>
    @include('pdf.partials.header', ['title' => 'Documents Report', 'logoBase64' => $logoBase64 ?? null])
        ...
        <tbody>
            @forelse ($rows as $row)
                <tr>
                    ...
                    <td>{{ $row->title ?? 'N/A' }}</td>
                    <td>{{ $row->type ?? 'N/A' }}</td>
                    <td>{{ $row->owner_name ?? $row->customer->name ?? 'N/A' }}</td>
                    <td>{{ ucwords((string) $row->status) }}</td>
                    <td>{{ $row->uploaded_at?->format('M d, Y') ?? 'N/A' }}</td>
                </tr>
            @empty
                <tr>
